  ...
  return (
    <div>
      ...
      <div className="flex gap-1 mb-6 bg-gray-100 p-1 rounded-xl w-fit">
        {(["fietsen", "types", "onderhoud"] as Tab[]).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
          ...
          >
            {t === "fietsen" ? "Fietsen" : t === "types" ? "Fietstypes" : `Onderhoud ${inMaintenance.length > 0 ? `(${inMaintenance.length})` : ""}`}
          </button>
        ))}
      </div>
          ...
          <div className="bg-white rounded-2xl shadow overflow-hidden">
            <table className="w-full text-sm text-left">
              <thead className="bg-gray-50 text-xs text-gray-500 uppercase tracking-wide">
                <tr>
                  <th className="py-3 px-4">Naam</th>
                  <th className="py-3 px-4">Prijs/dag</th>
              ...
              <tbody>
                {types.map((t) => (
                  <tr key={t.id} className="border-b">
                    ...
                    <td className="py-3 px-4">€{t.price_per_day.toFixed(2)}</td>
                    <td className="py-3 px-4 text-gray-500">{t.bike_count ?? 0}</td>
                  </tr>
                ))}
              </tbody>
            </table>
